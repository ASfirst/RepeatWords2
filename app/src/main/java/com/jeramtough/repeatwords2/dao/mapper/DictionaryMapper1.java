 ...
 * @deprecated because used outer database ,so this mapper is unusable.
 */
@Deprecated
@JtComponent
public class DictionaryMapper1 extends DaoMapper {

    @IocAutowire
    public DictionaryMapper1(MyDatabaseHelper myDatabaseHelper) {
    ...
    public void addWords(List<Word> words) {
        for (Word word : words) {
            ...
            getSqLiteDatabase()
                    .execSQL(sql,
                            new Object[]{word.getId(), word.getEn(), word.getCh(), word.getPhonetic()});
        }
    }
